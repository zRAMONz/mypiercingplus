...
function addToCartFromPopup() {
    ...
    var color = popup.querySelector('.popup-color').innerText;

    var measures = popup.querySelectorAll('.measure');
    for (var i = 0; i < measures.length; i++) {
        var price = parseFloat(measures[i].getAttribute('data-price'));
        var quantity = parseInt(measures[i].nextElementSibling.value, 10); // Corrigido aqui
        if (quantity > 0) {
            ...
            cartItem.innerText = productName + ' ' + color + ' ' + measure + ' - €' + price + ' * ' + quantity + ' unidades = €' + (price * quantity);
            ...
            // Depois de adicionar o produto ao carrinho, resetamos a variável selectedSize para null
            // para evitar que a seleção seja mantida após o popup ser fechado.
            selectedSize = null;
        }}
